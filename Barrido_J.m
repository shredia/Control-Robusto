...
J_factors = [0.5 1 2 5 10];   % multiplicadores respecto a J_internal
N = numel(J_factors);
...
for k = 1:N
    ...
    fprintf('\n=============================\n');
    fprintf('Simulando caso %d de %d | Jx%.3g\n', k, N, J_factors(k));
    fprintf('=============================\n');
    ...
    J_var = J_internal * J_factors(k);
    B_var = B_real;   % fijo, según tu archivo original
    ...
    data(k).name      = "Jx" + string(J_factors(k));
    data(k).J_factor  = J_factors(k);
    data(k).J_var     = J_var;
    data(k).B_var     = B_var;
    ...
    res(k).name     = name_k;
    res(k).J_factor = data(k).J_factor;
    res(k).J_var    = data(k).J_var;
...
summaryStruct(row).J_factor            = data(k).J_factor;
summaryStruct(row).J_var               = data(k).J_var;
...
save(fullfile(rootFolder, 'summaryTable.mat'), ...
    'summaryTable', 'J_factors', 'Wm_ref', 'fe_ref', 't_inicial', 't_final');
