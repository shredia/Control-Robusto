...
clear; clc;
syms R L Kt Va Vb Ia Ib We Wm Tx J Th_m Nr B a b Wa Wb % Variables simbólicas

% --- Ecuaciones del sistema ---
f1 = (Va - R*Ia + Kt*Wm*sin(Nr*Th_m))/L;                % d(Id)/dt
f2 = (Vb - R*Ib - Kt*Wm*cos(Nr*Th_m))/L;        % d(Iq)/dt
f3 = (Kt*(Ib*cos(Nr*Th_m)-Ia*sin(Nr*Th_m)) -B*Wm)/J;                         % d(Wm)/dt
f4 = Wm; 
...
v_no_obs = null(matriz_observabilidad)   % devuelve una base del subespacio no observable
